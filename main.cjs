...
// Backend URL: try baked config.json first (generated by scripts/bake-config.cjs
// at build time from .env), then VITE_BACKEND_URL / API_BACKEND_URL env vars,
// then packaged/production fallback, then dev localhost.
let BACKEND_URL = null;
// Only read config.json in packaged builds — avoids picking up stale
// production URLs in dev mode after a previous `npm run dist`.
if (app.isPackaged) {
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (config.BACKEND_URL) {
        ...
        console.log('🔧 BACKEND_URL from config.json:', BACKEND_URL);
      }
    }
  } catch (e) {
...
if (!BACKEND_URL) {
  BACKEND_URL = process.env.VITE_BACKEND_URL || process.env.API_BACKEND_URL || (app.isPackaged ? 'https://stealth-ai-production-bcbd.up.railway.app' : 'http://localhost:3001');
}
...
ipcMain.handle('google-auth-electron', async (event, backendUrlFromRenderer) => {
    ...
    try {
        // Prefer the baked config URL (from config.json / env), but allow the
        // renderer's URL as well (Vite bakes VITE_BACKEND_URL at build time).
