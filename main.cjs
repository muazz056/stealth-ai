...
ipcMain.handle('google-auth-electron', async (event, backendUrlFromRenderer) => {
    ...
    try {
        // Use BACKEND_URL directly (correctly resolved above). Ignore renderer's URL
        // when it's the localhost fallback (renderer's VITE_BACKEND_URL might not be
        // baked in at build time, so it defaults to 'http://localhost:3001').
        const backendUrl = (backendUrlFromRenderer && !backendUrlFromRenderer.includes('localhost:3001')) ? backendUrlFromRenderer : BACKEND_URL;
        console.log('🔧 Using backend URL for Google auth:', backendUrl);
