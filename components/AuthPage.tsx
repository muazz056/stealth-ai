...
const AuthPage: React.FC<AuthPageProps> = ({ onAuthSuccess }) => {
  ...
  const handleElectronGoogleAuth = async () => {
    try {
      setLoading(true);
      setError('');
  ...
  const handleCancelElectronGoogleAuth = async () => {
    try {
      const { ipcRenderer } = (window as any).require('electron');
      await ipcRenderer.invoke('google-auth-electron-cancel');
    } catch {}
    setLoading(false);
    setError('');
  };
    ...
    <div className="min-h-screen bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors duration-300 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative max-w-md w-full bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-2xl shadow-2xl">
        ...
        {isElectron ? (
          <div className="mb-6">
            <button
              onClick={handleElectronGoogleAuth}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
            >
            ...
            {loading && (
              <button
                onClick={handleCancelElectronGoogleAuth}
                className="mt-2 w-full text-xs text-red-500 hover:text-red-400 hover:underline text-center"
              >
                Cancel
              </button>
            )}
